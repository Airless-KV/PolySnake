...
public static class MainGameBootManager
{
    ...
    public static void AutoBoot()
    {
        ...
        // Load and apply system video/display settings at boot
        SystemSettings sysSettings = SystemSettings.Load();
        string[] resParts = sysSettings.Resolution.Split('x');
        if (resParts.Length == 2 && int.TryParse(resParts[0], out int w) && int.TryParse(resParts[1], out int h))
        {
            Screen.SetResolution(w, h, sysSettings.Fullscreen);
        }
        Application.targetFrameRate = sysSettings.FrameRateLimit;
        ...
        if (File.Exists(filePath))
        {
            ...
            CurrentDifficulty = string.IsNullOrEmpty(loadedSettings.Difficulty) ? "Normal" : loadedSettings.Difficulty;
        }
        else
        {
    ...
    private static void ApplyRulesToWorld()
    {
        ...
        referee.AddComponent<MainGameModeController>();

        GameObject scoreMgr = new GameObject("ScoreManager");
        scoreMgr.AddComponent<MainScoreManager>();

        if (MainSoundManager.Instance == null)
        {
            GameObject soundObj = new GameObject("SoundManager");
            soundObj.AddComponent<MainSoundManager>();
        }
        MainSoundManager.Instance.PlayMusicForDifficulty(CurrentDifficulty);

        if (MainInGameSettings.Instance == null)
        {
            GameObject settingsObj = new GameObject("InGameSettings");
            settingsObj.AddComponent<MainInGameSettings>();
        }
